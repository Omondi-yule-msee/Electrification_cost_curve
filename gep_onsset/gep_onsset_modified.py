...
class Technology:
    ...
    def __init__(self,
        ...
        self.discount_rate = discount_rate  # Code_updated

    @classmethod
    def set_default_values(cls, base_year, start_year, end_year, dummy, HV_line_type=69, HV_line_cost=53000,#Code_updated
        ...
        #cls.discount_rate = discount_rate #Code updated_remove cls
    ...
    def get_lcoe(self, energy_per_cell, people, num_people_per_hh, start_year, end_year, new_connections,
                 ...
                 get_capacity=False,get_salvage_cost = False,get_om_cost = False,get_lv_line_length = False, get_mv_line_length = False, get_capital_cost = False):
        """
        ...
        if self.grid_price > 0:
            ...
            if self.standalone and self.diesel_price == 0:
                try: #ETHUPDATE to omit zero division
                    if installed_capacity / (new_connections / num_people_per_hh) < 0.020:#ethupdate replaced "people" with "new_connections"
                        ...
                        CAPEX = self.capital_cost[0.020]


                    elif installed_capacity / (new_connections/ num_people_per_hh) < 0.050: #ethupdate replaced "people" with "new_connections"
                        ...
                        total_om_cost = td_om_cost + (self.capital_cost[0.050] * self.om_costs * conflict_sa_pen[
                            conf_status] * installed_capacity)
                        CAPEX = self.capital_cost[0.050]

                    elif installed_capacity / (new_connections / num_people_per_hh) < 0.100:#ethupdate replaced "people" with "new_connections"
                        ...
                        total_om_cost = td_om_cost + (self.capital_cost[0.100] * self.om_costs * conflict_sa_pen[
                            conf_status] * installed_capacity)
                        CAPEX = self.capital_cost[0.100]

                    elif installed_capacity / (new_connections / num_people_per_hh) < 1:#ethupdate replaced "people" with "new_connections"
                        ...
                        total_om_cost = td_om_cost + (self.capital_cost[5] * self.om_costs * conflict_sa_pen[
                            conf_status] * installed_capacity)
                        CAPEX = self.capital_cost[1]
                except ZeroDivisionError:  #ETHUPDATE to omit zero division
                    capital_investment = 0.00001
                    total_om_cost = 0.0000001
                    CAPEX = 0.000001
        ...
        if get_investment_cost:
            ...
            discounted_investments = investments
            if mg_hybrid:
            ...
            return (No_of_HV_LV_substation * self.HV_LV_sub_station_cost +
                    No_of_HV_MV_substation * self.HV_MV_sub_station_cost +
                    No_of_MV_MV_substation * self.MV_MV_sub_station_cost +
                    No_of_MV_LV_substation * self.MV_LV_sub_station_cost +
                    num_transformers * self.service_Transf_cost) * conf_grid_pen[conf_status] #Code_updated added num_transformers * self.service_Transf_cost to calculation so it includes everything
        elif get_investment_cost_connection:
            return total_nodes * self.connection_cost_per_hh
        elif get_capacity:
            return add_capacity
        elif get_lv_line_length:  # ETHUpdate added 06/03/2021
            return total_lv_lines_length  # ETHUpdate added 06/03/2021

        elif get_mv_line_length:  # ETHUpdate added 06/03/2021
            return mv_lines_connection_length  # ETHUpdate added 06/03/2021

        elif self.standalone and get_capital_cost:# ETHUpdate added 06/03/2021
            return CAPEX # ETHUpdate added 06/03/2021
        else:
...
class SettlementProcessor:
    ...
    def __init__(self, path):
        ...
        try:
            ...
            try:
                ...
                raise
    def Replace_PerCapitaDemand_Val(self): #Code_updated added this function
        """
                Do any initial data conditioning that may be required.
                """

        logging.info('Replace the PerCapitaDemand Values from source document to 0') #This is just to make sure the settlement data frame can add the necessary columns to the output csv
                    ...
                    while rural_elec_factor <= 1:
                        ...
                        if i < 10:
                            ...
                            rural_elec_modelled = self.df.loc[
                                (self.df[SET_ELEC_CURRENT] == 1) & (self.df[SET_URBAN] < 2), SET_ELEC_POP_CALIB].sum()
                            rural_elec_factor = rural_elec_modelled / rural_electrified
                            print ("Updated rural_electricication_factor"+str(i))
                            i += 1
                        else:
                            break
    ...
    def pre_electrification(self, grid_calc, grid_price, year, time_step, start_year):
        ...
        if (year - time_step) == start_year:
            ...
            self.df.loc[(self.df[SET_ELEC_FINAL_CODE + "{}".format(year - time_step)] == 1) & (
                ...
                year)] = 1
            #Set the final code column(SET_ELEC_FINAL_CODE) to equal = 1 meaning 'Grid electricity'
            ...
            if year >= grid_reach[unelec]:
                ...
                if dist_adjusted <= max_dist:
                    grid_lcoe = grid_calc.get_lcoe(energy_per_cell=enerperhh[unelec],
                                                   ...
                                                   elec_loop=0)
                    # This code prints for all sites that grids are viable but > mini_off_grid lcoe
    ...
    def final_decision(self, mg_hydro_calc, mg_wind_calc, mg_pv_calc, sa_pv_calc, mg_diesel_calc,
        ...
        def return_lv_line_length(row): # ETHUpdate added 06/03/2021
            min_code = row[SET_ELEC_FINAL_CODE + "{}".format(year)]
            if min_code == 1:
                return grid_calc.get_lcoe(energy_per_cell=row[SET_ENERGY_PER_CELL + "{}".format(year)],
                                          start_year=year - timestep,
                                          ...
                                          additional_mv_line_length=row[SET_MIN_GRID_DIST + "{}".format(year)],
                                          elec_loop=row[SET_ELEC_ORDER + "{}".format(year)],
                                          get_lv_line_length = True)
            else:
                return 0

        logging.info('Calculate lv_line distribution_length') # ETHUpdate added 06/03/2021
        self.df['Lv_line_length' + "{}".format(year)] = self.df.apply(return_lv_line_length, axis=1) #ETHUpdate added 06/03/2021

        def return_mv_line_length(row):# ETHUpdate added 06/03/2021
            min_code = row[SET_ELEC_FINAL_CODE + "{}".format(year)]
            if min_code == 1:
                return grid_calc.get_lcoe(energy_per_cell=row[SET_ENERGY_PER_CELL + "{}".format(year)],
                                          start_year=year - timestep,
                                          ...
                                          additional_mv_line_length=row[SET_MIN_GRID_DIST + "{}".format(year)],
                                          elec_loop=row[SET_ELEC_ORDER + "{}".format(year)],
                                          get_mv_line_length = True)
            else:
                return 0

        logging.info('Calculate mv_line_connection_length') # ETHUpdate added 06/03/2021
        self.df['mv_line_length' + "{}".format(year)] = self.df.apply(return_mv_line_length,axis=1) # ETHUpdate added 06/03/2021
        ...
        def return_capital_cost(row):  # # ETHUpdate added 06/03/2021
            min_code = row[SET_MIN_OVERALL_CODE + "{}".format(year)]
            if min_code == 3:
                return sa_pv_calc.get_lcoe(energy_per_cell=row[SET_ENERGY_PER_CELL + "{}".format(year)],
                                           start_year=year - timestep,
                                           ...
                                           capacity_factor=row[SET_GHI] / HOURS_PER_YEAR,
                                           conf_status=row[SET_CONFLICT],
                                           get_capital_cost=True)
            else:
                return 0

        logging.info('Calculate SAS CAPITAL COST')  # ETHUpdate added 06/03/2021
        self.df['sas_capital_cost' + "{}".format(year)] = self.df.apply(return_capital_cost, axis=1)  # ETHUpdate added 06/03/2021
