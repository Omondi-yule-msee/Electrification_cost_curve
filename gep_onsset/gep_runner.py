...
# Date: 05 June 2019
# Python version: 3.5
#Modified by Churchill Agutu 25 March 2020
# #All modifications by Churchill Agutu are noted as #ETHupdate
import os
from gep_onsset_modified import *
import pandas as pd
...
if choice == 1:
   ...
   SpecsData = pd.read_excel(specs_path, sheet_name='SpecsData') #ETHupdate changed from sheetname to sheet_name 
...
if choice == 3:
    ...
    ScenarioInfo = pd.read_excel(specs_path, sheet_name='ScenarioInfo') #ETHupdate changed from sheetname to sheet_name 
    Scenarios = ScenarioInfo['Scenario']
    ScenarioParameters = pd.read_excel(specs_path, sheet_name='ScenarioParameters')#ETHupdate changed from sheetname to sheet_name 
    SpecsData = pd.read_excel(specs_path, sheet_name='SpecsDataCalib')#ETHupdate changed from sheetname to sheet_name 
    Discount_rate_data = pd.read_excel(specs_path, sheet_name='DiscountRate') #ETHupdate  this line of code to import the discount rate
    print(SpecsData.loc[0, SPE_COUNTRY])

    #Confirm that all the correct documents are loaded for each country
    CountryCode_from_SpecData = SpecsData.iloc[0]['Country'] #ETHupdate 
    CountryCode_from_DiscountRate_sheet = Discount_rate_data.iloc[0]['Country'] #ETHupdate 
    Country_input_data = base_dir #ETHupdate 
    print ("Spec Data:",CountryCode_from_SpecData) #ETHupdate 
    print ("Discount Rate Country:",CountryCode_from_DiscountRate_sheet) #ETHupdate 
    print ("Country Source File:",Country_input_data) #ETHupdate 
    for scenario in Scenarios:
        print('Scenario: ' + str(scenario + 1))
        countryID = SpecsData.iloc[0]['CountryCode']
        print ('Country Name', countryID)
        ...
        onsseter = SettlementProcessor(settlements_in_csv)
        onsseter.Replace_PerCapitaDemand_Val() #ETHupdate  this here to convert PerCapitaDemand values to 0 for the code to initiate


        start_year = SpecsData.iloc[0][SPE_START_YEAR]
        end_year = SpecsData.iloc[0][SPE_END_YEAR]
        print ("SpecsData country",SpecsData.iloc[0]['Country'])

        existing_grid_cost_ratio = SpecsData.iloc[0][SPE_EXISTING_GRID_COST_RATIO] 
        num_people_per_hh_rural = float(SpecsData.iloc[0][SPE_NUM_PEOPLE_PER_HH_RURAL]) #ETHupdate
        num_people_per_hh_urban = float(SpecsData.iloc[0][SPE_NUM_PEOPLE_PER_HH_URBAN]) #ETHupdate
        max_grid_extension_dist = float(SpecsData.iloc[0][SPE_MAX_GRID_EXTENSION_DIST]) #ETHupdate
        urban_elec_ratio = float(SpecsData.iloc[0]['urban_elec_ratio_modelled']) #ETHupdate
        rural_elec_ratio = float(SpecsData.iloc[0]['rural_elec_ratio_modelled']) #ETHupdate
        annual_grid_cap_gen_limit = SpecsData.loc[0, 'NewGridGenerationCapacityAnnualLimitMW'] * 1000 #ETHupdate
        ...
        Technology.set_default_values(base_year=start_year,
                                      ...
                                      end_year=end_year,
                                      dummy = 0) #ETHupdate: Dummy here is just a place holder to ensure the code matches the previous one and is
        #not altered or does not impact the code in anyway
        grid_discount_rate = Discount_rate_data.iloc[0]['Grid']  #ETHupdate
        mg_discount_rate = Discount_rate_data.iloc[0]['MiniGrid']  # ETHupdate
        standalone_discount_rate = Discount_rate_data.iloc[0]['Standalonesystem'] #ETHupdate
        ...
        grid_calc = Technology(om_of_td_lines=0.02,
                               ...
                               discount_rate=grid_discount_rate) #ETHupdate
        ...
        mg_hydro_calc = Technology(om_of_td_lines=0.02,
                                   ...
                                   capital_cost=3000,
                                   om_costs=0.03,
                                   discount_rate=mg_discount_rate)  #ETHupdate

        mg_wind_calc = Technology(om_of_td_lines=0.02,
                                  distribution_losses=0.05,
                                  connection_cost_per_hh=100,
                                  base_to_peak_load_ratio=0.85,
                                  capital_cost=3750000000000,# ETHupdate - ignores MG wind
                                  om_costs=0.02,
                                  tech_life=20,
                                  discount_rate=mg_discount_rate)  #ETHupdate
        ...
        mg_pv_calc = Technology(om_of_td_lines=0.02,
                                ...
                                tech_life=20,
                                om_costs=0.015,
                                capital_cost=2139 * pv_capital_cost_adjust,
                                discount_rate=mg_discount_rate)   # ETHupdate

        sa_pv_calc = Technology(base_to_peak_load_ratio=0.9,
                                tech_life=15,
                                om_costs=0.02,
                                capital_cost={0.020: 6720 * pv_capital_cost_adjust,
                                              0.050: 6133 * pv_capital_cost_adjust,
                                              0.100: 4457 * pv_capital_cost_adjust,
                                              1: 3122 * pv_capital_cost_adjust,
                                              5: 3122 * pv_capital_cost_adjust},
                                standalone=True,
                                discount_rate=standalone_discount_rate)   #ETHupdate
        ...
        # gmancini: I would delete all of this which is commented
        ...
        elements = ["1.Population", "2.New_Connections",
                    "3.Capacity", "4.Investment"]
        techs = ["Grid", "SA_Diesel", "SA_PV", "MG_Diesel",
                 "MG_PV", "MG_Wind", "MG_Hydro", "MG_Hybrid"]
        ...
        for year in yearsofanalysis:
            ...
            hybrid_1 = pv_diesel_hyb.pv_diesel_hybrid(1, max(onsseter.df[SET_GHI]),
                                                      max(onsseter.df[SET_TRAVEL_HOURS]
                                                          ), 1, year - time_step, end_year,
                                                      pv_no=pv_no, diesel_no=diesel_no)
            hybrid_2 = pv_diesel_hyb.pv_diesel_hybrid(1, max(onsseter.df[SET_GHI]),
                                                      max(onsseter.df[SET_TRAVEL_HOURS]
                                                          ), 2, year - time_step, end_year,
                                                      pv_no=pv_no, diesel_no=diesel_no)
            hybrid_3 = pv_diesel_hyb.pv_diesel_hybrid(1, max(onsseter.df[SET_GHI]),
                                                      max(onsseter.df[SET_TRAVEL_HOURS]
                                                          ), 3, year - time_step, end_year,
                                                      pv_no=pv_no, diesel_no=diesel_no)
            hybrid_4 = pv_diesel_hyb.pv_diesel_hybrid(1, max(onsseter.df[SET_GHI]),
                                                      max(onsseter.df[SET_TRAVEL_HOURS]
                                                          ), 4, year - time_step, end_year,
                                                      pv_no=pv_no, diesel_no=diesel_no)
            hybrid_5 = pv_diesel_hyb.pv_diesel_hybrid(1, max(onsseter.df[SET_GHI]),
                                                      max(onsseter.df[SET_TRAVEL_HOURS]
                                                          ), 5, year - time_step, end_year,
                                                      pv_no=pv_no, diesel_no=diesel_no)
            ...
            onsseter.calculate_off_grid_lcoes(mg_hydro_calc, mg_wind_calc, mg_pv_calc, sa_pv_calc, mg_diesel_calc,
                                              sa_diesel_calc, hybrid_1, hybrid_2, hybrid_3, hybrid_4,
                                              hybrid_5, year, start_year, end_year, time_step, diesel_techs=0)  #update #exclude diesel generation

            onsseter.pre_electrification(
                grid_calc, grid_price, year, time_step, start_year)
            ...
            onsseter.apply_limitations(
                eleclimit, year, time_step, prioritization, auto_intensification)
        ...
        onsseter.df['FinalElecCode' +
                    str(year)] = onsseter.df['FinalElecCode' + str(year)].astype(int)

        for i in range(len(onsseter.df.columns)):
            if onsseter.df.iloc[:, i].dtype == 'float64':
                onsseter.df.iloc[:, i] = pd.to_numeric(
                    onsseter.df.iloc[:, i], downcast='float')
            elif onsseter.df.iloc[:, i].dtype == 'int64':
                onsseter.df.iloc[:, i] = pd.to_numeric(
                    onsseter.df.iloc[:, i], downcast='signed')
